...
export default function NotesApp() {
    ...
    const [view, setView] = useState<'folders' | 'notes' | 'editor'>('folders');
    ...
    const handleBack = () => {
        if (view === 'editor') setView('notes');
        else if (view === 'notes') setView('folders');
    };
    ...
    return (
        <AppContainer style={{ flexDirection: isMobile ? 'column' : 'row' }}>
            ...
            {(!isMobile || view === 'folders') && (
                <FoldersColumn style={{ width: isMobile ? '100%' : '180px' }}>
                    <div style={{ padding: '4px', borderBottom: '2px solid silver', marginBottom: '4px' }}>
                       <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                if (selectedTopicId && confirm('Delete folder and all notes?')) {
            ...
            {/* Column 2: Notes */}
            {(!isMobile || view === 'notes') && (
                <NotesColumn style={{ width: isMobile ? '100%' : '220px' }}>
                    <div style={{ padding: '4px', borderBottom: '2px solid silver', marginBottom: '4px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                            {isMobile && <Button onClick={handleBack} size="sm">←</Button>}
                            <Button
                                disabled={!selectedTopicId}
                ...
                <EditorColumn style={{ flex: 1 }}>
                    {selectedNote ? (
                        ...
                        <div style={{ display: 'flex', height: '100%', alignItems: 'center', justifyContent: 'center', backgroundColor: '#c0c0c0', color: 'black', textAlign: 'center', padding: '20px' }}>
                            {isMobile && <Button onClick={handleBack} style={{ position: 'absolute', top: 10, left: 10 }}>← Back</Button>}
